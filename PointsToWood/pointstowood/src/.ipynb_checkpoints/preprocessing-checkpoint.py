...
class Voxelise:
    ...
    def write_voxels(self):
        ...
        for _, voxel_indices in enumerate(tqdm(voxels, desc='Writing voxels')):
            ...
            vxl_info = {'voxel_id' : file_counter,'shape':voxel.shape}
            voxels_data.append({'voxel_id' : file_counter,'shape':list(voxel.shape)})
            torch.save(voxel, os.path.join(self.vxpath, f'voxel_{file_counter}.pt'))
            file_counter += 1
        df = pd.DataFrame.from_records(voxels_data)
        df.to_csv('info_voxels.csv',sep=';')
        return self.pos[:, -1]
