...
def SemanticTraining(args):
    ...
    # Función Loss
    criterion = Poly1FocalLoss(reduction="mean", gamma = 2.0, alpha = None, label_smoothing = 0.1) #graficar
    
    # Probar Fine Tuning
    if args.tune: # cambio en learning rate
        first_cycle = max(6
                          , args.num_epochs // 5)
        optimizer = AdamW(filter(lambda p: p.requires_grad, model.parameters()), lr=1e-5, weight_decay=1e-2)
        lr_scheduler = CosineAnnealingWarmupRestarts(optimizer, first_cycle_steps=first_cycle, cycle_mult=1.0, max_lr=1e-6, min_lr=1e-8, warmup_steps=5, gamma=0.5)
    else:
    ...
    for epoch in range(1, args.num_epochs + 1):
        ...
        with tqdm(total=len(train_loader), colour='white', ascii="░▒", bar_format='{l_bar}{bar:20}{r_bar}{bar:-20b}') as tepoch:
            for i, (data) in enumerate(train_loader):
                ...
                try:
                    ...
                    #    valor = float(loss.cpu())

                    #    loss_values_arr.append(valor)
            ...
            loss_values_epoch.append(epoch_loss)

            print(f"Loss del Epoch {epoch}: {epoch_loss:.5f}")
            lr_scheduler.step()
        ...
        if args.test:
            ...
            model.eval()
            test_loss = 0.0  # acumulador del loss de validación
            sleep(0.1)
            ...
            with tqdm(total=len(test_loader), colour='white', ascii="▒█", bar_format='{l_bar}{bar:20}{r_bar}{bar:-20b}') as tepoch:
                with torch.no_grad():
                    for j, data in enumerate(test_loader):
                        ...
                        # === CALCULAR LOSS DE VALIDACION ===
                        loss_test_batch, gamma = criterion(outputs, data.y)
                        test_loss += loss_test_batch.item()

                        # === PREDICCIONES ===
                        probs = torch.sigmoid(outputs)
                        preds = (probs >= 0.50).type(torch.int64)
                        ...
                        test_accuracy += balanced_accuracy_score(data.y.cpu(), preds.cpu())
                        test_f1 += f1_score(data.y.cpu(), preds.cpu(), average='binary', zero_division=0)

                        tepoch.set_description(f"Test")
                        tepoch.update()
                        tepoch.set_postfix({
                            'Lo': np.around(test_loss / (j + 1), 5),   # <-- pérdida promedio por batch
                ...
                tepoch.close()
            val_loss_epoch = test_loss / len(test_loader)
            val_loss_values_epoch.append(val_loss_epoch)
    ...
    eje_x = range(1, len(loss_values_epoch) + 1)
    eje_x_val = range(1, len(val_loss_values_epoch) + 1)
    print(eje_x_val)
    print(val_loss_values_epoch)

    plt.figure(figsize=(10,6))

    # Curva de entrenamiento
    plt.plot(eje_x, loss_values_epoch, marker='o', linestyle='-', color='b', label='Train Loss')

    # Curva de validación
    plt.plot(eje_x_val, val_loss_values_epoch, marker='+', linestyle='-', color='g', label='Validation Loss')

    plt.title('Gráfico de función Loss por Epoch')
    plt.xlabel('Epochs')
    plt.ylabel('Loss')
    plt.legend()
    ...
    # plt.savefig(f"loss_{now_datetime}.png", dpi=300)
    plt.savefig(f'grafico_{str(now_datetime)}.png', dpi=300, bbox_inches='tight')
