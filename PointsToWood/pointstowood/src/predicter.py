...
class TestingDataset(Dataset, ABC):
    ...
    def __getitem__(self, index):
        ...
        # Caso: point cloud vacío
        if point_cloud.numel() == 0 or point_cloud.shape[0] == 0:
            print(f"Warning: Empty point cloud at index {index}")
            pos = torch.empty((0, 3), dtype=torch.float)
            reflectance = torch.empty((0,), dtype=torch.float)
            local_shift = torch.zeros(3, dtype=torch.float)
            scaling_factor = torch.tensor(1.0, dtype=torch.float)

            return Data(pos=pos, reflectance=reflectance, local_shift=local_shift, sf=scaling_factor)
    ...
    def collect_predictions(self, classification, original):
        ...
        if os.path.exists(indices_file):
            ...
            for i in range(0, len(original), batch_size):
                chunk = original.values[i:i+batch_size, :3]
                _, indices = kd_tree.query(chunk, k=32)
                results.append(indices)
            indices = np.vstack(results)
            # _, indices = kd_tree.query(original.values[:, :3], k = 32 if self.any_wood != 1 else 64)
        
        classification = classification.astype(np.float32)
        labels = np.zeros((original.shape[0], 2), dtype=np.float32)
